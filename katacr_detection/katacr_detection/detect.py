...
path_detectors = [
  ...
  path_root / './katacr_detection/katacr_detection/models/detector1_v0.7.13.pt',
  path_root / './katacr_detection/katacr_detection/models/detector2_v0.7.13.pt',
]

class ComboDetector:
  def __init__(self, path_detectors, show_conf=True, conf=0.7, iou_thre=0.6, tracker='bytetrack', enable_coordinate_filtering=True):
    self.models = [YOLO_CR(str(p)) for p in path_detectors]
    self.show_conf, self.conf = show_conf, conf
    self.iou_thre = iou_thre
    self.tracker = None
    self.enable_coordinate_filtering = enable_coordinate_filtering
  ...
  def infer(self, x, pil=False):
    ...
    if self.enable_coordinate_filtering:
  ...
  def predict(self, source, show=False, save=True, video_interval=1):
    ...
    open_window = False
    path_save_result = path_root / f"logs/detection" / time.strftime("%Y_%m_%d_%H_%M_%S")
    if save:
